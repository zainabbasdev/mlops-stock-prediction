...
class StockDataTransformer:
    ...
    def create_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['momentum_10'] = df[self.target_column] - df[self.target_column].shift(10)
        ...
        if 'volume' in df.columns:
            ...
            self.feature_names.extend(['volume_ma_5', 'volume_ma_20', 'volume_ratio'])
    ...
    def create_target_variable(self, df: pd.DataFrame, horizon: int = 1) -> pd.DataFrame:
        ...
        # Alternative: realized volatility over next N periods
        df['target_realized_vol'] = df['returns'].shift(-horizon).rolling(window=horizon).std().shift(-horizon+1)
